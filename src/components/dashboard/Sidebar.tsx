...
const navItems = [
  ...
  { name: 'Quote Requests', path: '/dashboard/quote-requests', icon: FileText, badge: 3 },
    ...
    <aside className="w-[260px] bg-(--color-surface) border-r border-(--color-border) flex flex-col h-full">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.badge && <span className="bg-(--color-primary) text-white text-[0.75rem] px-2 py-0.5 rounded-full font-semibold">{item.badge}</span>}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="px-4 py-4 border-t border-(--color-border)">
        <div className="flex items-center p-2">
          <img className="w-10 h-10 rounded-full mr-3" src="https://ui-avatars.com/api/?name=Sarah+Mitchell&background=c25e28&color=fff" alt="Sarah Mitchell" />
          <div className="flex-1 flex flex-col">
            <span className="text-sm font-semibold text-(--color-text-main)">Sarah Mitchell</span>
            <span className="text-xs text-(--color-text-muted)">Super Admin</span>
          </div>
          <button className="text-(--color-text-muted) p-2 transition-colors duration-200 hover:text-(--color-text-main)" onClick={() => router.push('/sign-in')}>
            <LogOut size={18} />
          </button>
        </div>
      </div>
